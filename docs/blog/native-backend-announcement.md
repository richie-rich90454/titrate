...
Today we are shipping the LLVM native backend for Titrate. After four
...
This post covers what we built, why we built it, how it works at a high level and what is coming next.
...
The bytecode VM is fast enough for most programs. It is a switch-based
...
The native backend is functional and fast, but there is more to do:
...
- **More platforms** — we test on x86-64 Linux and Windows. ARM64
  (Apple Silicon, Graviton) should work but is not tested.
...
you (in either direction), we would love to hear about it.
