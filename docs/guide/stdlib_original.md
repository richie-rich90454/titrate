...
The Titrate standard library (`tt`) is a comprehensive collection of modules that provide essential functionality for everyday programming 鈥?from collections and I/O to math, networking, and testing. It is shipped with every Titrate installation and requires no external dependencies.
...
- **`::` is only used in import statements** 鈥?everywhere else, use `.` for member access
...
### heapq 鈥?Heap-Based Priority Queue
...
### dataclass 鈥?Auto-Generated Class Boilerplate
...
### contextlib 鈥?Resource Management
...
Contextlib.suppress(fn(): void {
    // risky operation 鈥?errors are silently ignored
});
```
...
### statistics 鈥?Statistical Functions
...
### chemistry 鈥?Computational Chemistry
...
### datetime 鈥?Date, Time, and Duration
...
let parsed: DateTime = DateTime.parse("2026-01-15T10:30:00");
let formatted: string = DateTime.format(now, "yyyy-MM-dd");
```

## Random & Utilities

### functools 鈥?Higher-Order Functions
...
### argparse 鈥?Command-Line Argument Parsing
...
### algorithms 鈥?Common Algorithms
...
### testing 鈥?Built-in Testing Framework (Assay)
...
鈹溾攢鈹€ Yes 鈫?HashMap<K, V>
鈹斺攢鈹€ No
    鈹溾攢鈹€ Do you need unique elements?
    鈹?  鈹溾攢鈹€ Yes 鈫?Set<T>
    鈹?  鈹斺攢鈹€ No
    鈹?      鈹溾攢鈹€ Do you need priority ordering?
    鈹?      鈹?  鈹溾攢鈹€ Yes 鈫?PriorityQueue<T>
    鈹?      鈹?  鈹斺攢鈹€ No
    鈹?      鈹?      鈹溾攢鈹€ Do you need FIFO/LIFO operations?
    鈹?      鈹?      鈹?  鈹溾攢鈹€ Yes 鈫?Deque<T>
    鈹?      鈹?      鈹?  鈹斺攢鈹€ No 鈫?ArrayList<T>
    鈹?      鈹?      鈹斺攢鈹€ Do you need frequency counting?
    鈹?      鈹?          鈹斺攢鈹€ Yes 鈫?Counter<T>
    鈹斺攢鈹€ Do you need efficient string building?
        鈹斺攢鈹€ Yes 鈫?StringBuilder
```
...
| `StringBuilder` | O(1) append | 鈥?| 鈥?| 鈥?| Yes |
