...
- `Array<T>(int size)` — create an array of the given size, filled with null
- `Array<T>.of(T... elements): Array<T>` — create from varargs
- `Array<T>.filled(int size, T value): Array<T>` — create an array filled with a value
...
- `forEach(action: function<void(T)>): void` — iterate with side effect
- `map(mapper: function<T(T)>): Array<T>` — transform each element into a new Array
...
arr.set(0, 10);
arr.set(1, 20);
io::println(arr.get(0));   // 10
io::println(arr.size());   // 5

let filled = Array<int>::filled(3, 0);
filled.fill(7);
io::println(filled.get(1));  // 7

let doubled = filled.map(fn(n: int): int => n * 2);
io::println(doubled.get(0)); // 14
```
