...
impl Compiler {
    ...
    fn decode_instructions(chunk: &Chunk) -> Vec<DecodedInstr> {
    ...
    fn encode_instructions(chunk: &mut Chunk, instructions: &[DecodedInstr]) {
    ...
    fn fold_constants_chunk(chunk: &mut Chunk) {
    ...
    fn eliminate_dead_code_chunk(chunk: &mut Chunk) {
        let old_instructions = Self::decode_instructions(chunk);
