...
impl Compiler {
    ...
    pub(super) fn compile_class_methods(&mut self, class_decl: &ast::ClassDecl) -> Result<(), String> {
        ...
        // Each field with an init expression gets its own Chunk stored in
        // the class's `field_inits` slot. The VM executes these chunks
        // during `exec_new` to populate the instance's fields before the
        // constructor runs.
        for member in &class_decl.members {
            if let ast::ClassMember::Field(field_decl) = member {
                if let Some(ref init_expr) = field_decl.init {
                    // Compile the init expression into a temporary function
                    // so we can use the standard compile_expr machinery,
                    // then move the compiled chunk into the class's
                    // `field_inits` slot (replacing the placeholder).
                    ...
                    // Move the compiled chunk out of the temp function and
                    // into the matching field_inits slot on the class.
