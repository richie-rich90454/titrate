        ...
        match expr {
            ...
            ast::Expr::Identifier(name, _) => {
                match scope.borrow().lookup(name) {
                    ...
                    Some(Symbol::Variant { enum_name, .. }) => ast::Type::simple(&enum_name),
                            ...
                            if let ast::ClassMember::Method(m) = member {
                                if m.name == method_name {
                                    return m
                                        .return_type
                                        .clone()
                                        .unwrap_or(ast::Type::simple("unknown"));
                                }
                            }
                        }
                    }
                ...
                match callee.as_ref() {
                    ast::Expr::Identifier(name, _) => {
                        if let Some(Symbol::Function(f)) = scope.borrow().lookup(name) {
                            f.return_type.clone().unwrap_or(ast::Type::simple("void"))
                        } else if let Some(Symbol::Variant { enum_name, .. }) =
                        ...
                        if let ast::Expr::Identifier(class_name, _) = obj.as_ref() {
                            if let Some(Symbol::Variable { .. }) = scope.borrow().lookup(class_name)
                            {
                                ...
                                let result = match (class_name.as_str(), method.as_str()) {
                                    ("Integer", "parseInt") | ("Integer", "parseOr") => {
                                        Some(ast::Type::simple("int"))
                                    }
                                    ("Double", "parse") | ("Double", "parseDouble") => {
                                        Some(ast::Type::simple("double"))
                                    }
                                    ("Long", "parseLong") => Some(ast::Type::simple("long")),
                                    ("Float", "toF32Bits") | ("Float", "fromF32Bits") => {
                                        Some(ast::Type::simple("float"))
                                    }
                                    ...
                                    ("String", "toUpperCase") | ("String", "toLowerCase") => {
                                        Some(ast::Type::simple("string"))
                                    }
                                    ("String", "trim")
                                    | ("String", "trimStart")
                                    | ("String", "trimEnd") => Some(ast::Type::simple("string")),
                                    ("String", "startsWith")
                                    | ("String", "endsWith")
                                    | ("String", "contains") => Some(ast::Type::simple("bool")),
                                    ("String", "replace") => Some(ast::Type::simple("string")),
                                    ("String", "split") => Some(ast::Type::generic(
                                        "ArrayList",
                        ...
                        // Handle generic container methods (ArrayList.get, HashMap.get, etc.)
                        {
                            ...
                            match (type_name, method.as_str()) {
                                ("ArrayList", "size") | ("ArrayList", "length") => {
                                    return ast::Type::simple("int")
                                }
                                ("ArrayList", "isEmpty") | ("ArrayList", "contains") => {
                                    return ast::Type::simple("bool")
                                }
                                ("ArrayList", "indexOf") => return ast::Type::simple("int"),
                                ("HashMap", "size") | ("HashMap", "length") => {
                                    return ast::Type::simple("int")
                                }
                                ("HashMap", "isEmpty")
                                | ("HashMap", "containsKey")
                                | ("HashMap", "containsValue") => return ast::Type::simple("bool"),
                                _ => {}
                            }
                            if let Some(first_param) = obj_type.params().first() {
                                match (type_name, method.as_str()) {
                                    ("ArrayList", "get") => return first_param.clone(),
                                    ("ArrayList", "first") | ("ArrayList", "last") => {
            ...
            ast::Expr::RefExpr(inner, ref_kind, _) => {
                let inner_type = self.infer_expr_type(inner, scope);
                match ref_kind {
                    ast::RefKind::Immutable => ast::Type::Ref(Box::new(inner_type)),
                    ast::RefKind::Mutable => ast::Type::MutRef(Box::new(inner_type)),
                }
            }
            ...
            ast::Expr::StaticCall {
                ...
                match (class_name.as_str(), method.as_str()) {
                    ...
                    ("String", "split") => {
                        ast::Type::generic("ArrayList", vec![ast::Type::simple("string")])
                    }
                    ...
                    ("Math", "abs") | ("Math", "absInt") | ("Math", "fabs") => {
                        ast::Type::simple("double")
                    }
                    ...
                    | ("Math", "ulp")
                    | ("Math", "scalb")
                    | ("Math", "fma") => ast::Type::simple("double"),
                    ("Math", "getExponent") => ast::Type::simple("int"),
                    // MathAdvanced
                    ("MathAdvanced", "sqrt") | ("MathAdvanced", "pow") => {
                        ast::Type::simple("double")
                    }
                    ("MathAdvanced", "exp") | ("MathAdvanced", "ln") => ast::Type::simple("double"),
                    ("MathAdvanced", "log2") | ("MathAdvanced", "log10") => {
                        ast::Type::simple("double")
                    }
                    ("MathAdvanced", "cbrt") | ("MathAdvanced", "hypot") => {
                        ast::Type::simple("double")
                    }
                    // MathTrig
                    ("MathTrig", "sin") | ("MathTrig", "cos") | ("MathTrig", "tan") => {
                        ast::Type::simple("double")
                    }
                    ("MathTrig", "asin")
                    | ("MathTrig", "acos")
                    | ("MathTrig", "atan")
                    | ("MathTrig", "atan2") => ast::Type::simple("double"),
                    ("MathTrig", "sinh") | ("MathTrig", "cosh") | ("MathTrig", "tanh") => {
                        ast::Type::simple("double")
                    }
                    // Hash methods
                    ("Hash", "md5")
                    ...
                    | ("Hash", "sha256")
                    | ("Hash", "sha384")
                    | ("Hash", "sha512") => ast::Type::simple("string"),
                    ("Hash", "sha3_256") | ("Hash", "sha3_384") | ("Hash", "sha3_512") => {
                        ast::Type::simple("string")
                    }
                    ("Hash", "blake2b") | ("Hash", "blake2s") | ("Hash", "crc32") => {
                        ast::Type::simple("string")
                    }
                    // Encoding methods
                    ...
                    ("File", "readBytes") => {
                        ast::Type::generic("ArrayList", vec![ast::Type::simple("byte")])
                    }
                    ...
                    | ("Path", "basename")
                    | ("Path", "dirname")
                    | ("Path", "extension") => ast::Type::simple("string"),
                    ("Path", "exists")
                    | ("Path", "isFile")
                    | ("Path", "isDir")
                    | ("Path", "isSymlink") => ast::Type::simple("bool"),
                    // Dir methods
                    ("Dir", "list") => {
                        ast::Type::generic("ArrayList", vec![ast::Type::simple("string")])
                    }
                    ...
                    | ("Time", "perfCounter")
                    | ("Time", "epochSeconds")
                    | ("Time", "nanos") => ast::Type::simple("long"),
                    ...
                    | ("Time", "getMinute")
                    | ("Time", "getSecond")
                    | ("Time", "dayOfYear") => ast::Type::simple("int"),
                    ...
                    | ("Regex", "subN")
                    | ("Regex", "findWithFlags")
                    | ("Regex", "matchWithFlags") => ast::Type::simple("string"),
                    ...
                    | ("Os", "hostName")
                    | ("Os", "release")
                    | ("Os", "version") => ast::Type::simple("string"),
                    ("Env", "set")
                    | ("Env", "unset")
                    | ("Os", "setenv")
                    | ("Os", "unsetenv")
                    ...
                    | ("Os", "utime")
                    | ("Os", "link")
                    | ("Os", "system") => ast::Type::simple("void"),
                    ("Os", "exists") | ("Os", "isFile") | ("Os", "isDir") | ("Os", "access") => {
                        ast::Type::simple("bool")
                    }
                    ("Os", "size")
                    | ("Os", "cpuCount")
                    | ("Os", "getpid")
                    | ("Os", "getppid")
                    | ("Os", "umask") => ast::Type::simple("long"),
                    ("Os", "scandir") => {
                        ast::Type::generic("ArrayList", vec![ast::Type::simple("Variant")])
                    }
                    ...
                    | ("Sys", "exit")
                    | ("Sys", "sleep")
                    | ("Sys", "changeDir") => ast::Type::simple("void"),
                    // Process methods
                    ("Process", "id") | ("Process", "args") => ast::Type::simple("long"),
                    ("Process", "spawn") | ("Process", "join") | ("Process", "terminate") => {
                        ast::Type::simple("void")
                    }
                    // Thread methods
                    ("Thread", "spawn")
                    | ("Thread", "spawnRunnable")
                    | ("Thread", "join")
                    | ("Thread", "sleep")
                    | ("Thread", "yield")
                    | ("Thread", "detach") => ast::Type::simple("void"),
                    ...
                    ("Sqlite", "execute") | ("Sqlite", "close") | ("Sqlite", "closeResult") => {
                        ast::Type::simple("void")
                    }
                    ("Sqlite", "query") | ("Sqlite", "nextRow") => ast::Type::simple("bool"),
                    ("Sqlite", "lastInsertId")
                    | ("Sqlite", "getInt")
                    | ("Sqlite", "columnCount") => ast::Type::simple("int"),
                    ...
                    ("Subprocess", "run")
                    | ("Subprocess", "exec")
                    | ("Subprocess", "popenWrite") => ast::Type::simple("string"),
                    // Gzip / Zlib methods
                    ("Gzip", "compress")
                    | ("Gzip", "decompress")
                    | ("Zlib", "compress")
                    | ("Zlib", "decompress") => ast::Type::simple("string"),
                    // Zip methods
                    ("ZipFile", "open") => ast::Type::simple("long"),
                    ("ZipFile", "entryCount") => ast::Type::simple("int"),
                    ("ZipFile", "entryName") => ast::Type::simple("string"),
                    ("ZipFile", "readEntry") | ("ZipFile", "extractAll") => {
                        ast::Type::simple("string")
                    }
                    ...
                    | ("Socket", "createServer") => ast::Type::simple("long"),
                    ("Socket", "connect")
                    | ("Socket", "bind")
                    | ("Socket", "listen")
                    | ("Socket", "accept") => ast::Type::simple("void"),
                    ...
                    | ("Socket", "setBroadcast")
                    | ("Socket", "setKeepAlive")
                    | ("Socket", "setLinger") => ast::Type::simple("void"),
                    ...
                    | ("Socket", "inetPton")
                    | ("Socket", "inetNtop") => ast::Type::simple("string"),
                    // Net methods
                    ("Net", "connect") | ("Net", "bind") | ("Net", "accept") => {
                        ast::Type::simple("long")
                    }
                    ("Net", "send")
                    | ("Net", "receive")
                    | ("Net", "getLocalPort")
                    | ("Net", "getRemotePort") => ast::Type::simple("int"),
                    ("Net", "getLocalAddress") | ("Net", "getRemoteAddress") => {
                        ast::Type::simple("string")
                    }
                    ...
                    | ("Http", "delete")
                    | ("Http", "patch")
                    | ("Http", "head") => ast::Type::simple("string"),
                    ("Http", "setTimeout") | ("Http", "setFollowRedirects") => {
                        ast::Type::simple("void")
                    }
                    // Dns methods
            ...
            ast::Expr::Assign(_target, value, _) => self.infer_expr_type(value, scope),
            ast::Expr::Ternary {
                then_expr,
                else_expr,
                ..
